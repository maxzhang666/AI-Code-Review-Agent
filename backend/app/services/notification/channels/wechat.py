...
class WechatWorkService:
    ...
    async def send_text(self, content: str) -> NotificationResult:
        if not self._webhook_url:
            return {"success": False, "message": "WeChat webhook_url is not configured", "response_time": 0.0, "details": {}}

        started = time.perf_counter()
        payload = {
            "msgtype": "text",
            "text": {"content": truncate_text(content, 1500)},
        }
        request_payload = {
        ...
        try:
            ...
            return {
                "success": success,
                "message": "WeChat sent" if success else f"WeChat failed: {data.get('errmsg', 'unknown error')}",
            ...
            elapsed = time.perf_counter() - started
            self._logger.log_error_with_context("wechat_send_failed", error=exc)
            return {
                "success": False,
                "message": f"WeChat exception: {exc}",
                "response_time": elapsed,
                "details": {
                    "request": request_payload,
